class Horses::Horse::RaceQualityCalculator
  ...
  def run
    ...
    if stats.consistency < Config::Racing.stats[:constistency][:max] * 0.2
      quality -= 20
    elsif stats.consistency < Config::Racing.stats[:constistency][:max] * 0.4
      quality -= 10
    elsif stats.consistency > Config::Racing.stats[:constistency][:max] * 0.8
      quality += 20
    elsif stats.consistency > Config::Racing.stats[:constistency][:max] * 0.6
      quality += 10
    end

    tracks = [stats.dirt, stats.turf, stats.steeplechase]
    ave_track = tracks.sum(0.0) / stats.size
