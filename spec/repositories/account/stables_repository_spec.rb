...
RSpec.describe Account::StablesRepository do
  describe "#active" do
    it "returns stables for active users" do
      ...
      result = described_class.new(model: Account::Stable).active
  ...
  describe "#ordered_by_name" do
    it "returns stables in ascending order" do
      expect(described_class.new(model: Account::Stable).ordered_by_name).to eq(Account::Stable.order(name: :asc))
    end
  end
end
