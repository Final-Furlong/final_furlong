module Racing
  class EntryCreator
    attr_reader :race, :horse, :race_location, :shipping_mode

    def create_entry(race:, horse:, stable:, first_jockey: nil, second_jockey: nil, third_jockey: nil, racing_style: nil, equipment: 0, ship_mode: nil)
      @race = race
      @horse = horse
      @shipping_mode = ship_mode.to_s.inquiry
      race_date = Date.parse(race.date.to_s)
      entry = Racing::RaceEntry.new
      ...
      if data.location != race_location && !data.in_transit
        if shipping_mode.present?
          route = Shipping::Route.with_locations(race_location, data.location).first
          costs = []
          days = []
          if shipping_mode.road? && route.road_days && route.road_days < max_travel_days
            costs << route.road_cost
            days << route.road_days
          end
          if shipping_mode.air? && route.air_days && route.air_days < max_travel_days
      ...
      attrs[:first_jockey] = Racing::Jockey.find_by(id: first_jockey) if first_jockey.present?
      attrs[:second_jockey] = Racing::Jockey.find_by(id: second_jockey) if second_jockey.present?
      attrs[:third_jockey] = Racing::Jockey.find_by(id: third_jockey) if third_jockey.present?
      entry.assign_attributes(attrs)
      ActiveRecord::Base.transaction do
        if needs_shipment && shipping_mode.present?
    ...
    def shipment_params
      { departure_date: Date.current, ending_location: race_location.id, mode: shipping_mode }
    end
