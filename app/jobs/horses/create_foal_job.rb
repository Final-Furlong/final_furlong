class Horses::CreateFoalJob < ApplicationJob
  ...
  attr_reader :breeding, :stable, :premature, :inbreeding
  ...
  def perform(id:)
    ...
    ActiveRecord::Base.transaction do
      ...
      if breeding.event == "birth" || breeding.event == "death" || breeding.event.starts_with?("twins")
        generate_future_events(gender)
        foal_quality = determine_foal_quality
        horse.reload
        generate_racing_stats(horse:, quality: foal_quality)
        # breeding stats
        # race options
        # racehorse metadata
        breeding.update(first_foal: horse)
      ...
      if breeding.event.starts_with?("twins")
        ...
        generate_future_events(gender)
        foal_quality = determine_foal_quality
        generate_racing_stats(horse: twin, quality: foal_quality)
        # breeding stats
        # race options
        # racehorse metadata
        breeding.update(first_foal: horse, second_foal: twin)
  ...
  def generate_future_events(gender)
    retire_date = generate_retirement_date(gender)
  ...
  def generate_retirement_date(gender)
  ...
  def generate_racing_stats(horse:, quality:)
    ...
    Racing::RacingStats.create!(
      ...
      desired_equipment: pick_equipment,
      ...
      peak_end_date: generate_peak_end_date(horse, quality),
      peak_start_date: generate_peak_start_date(horse, quality),
  ...
  def generate_peak_end_date(horse, quality)
  end

  def generate_peak_start_date(horse, quality)
  end
  ...
  def generate_stat(stat, horse, quality)
    ...
    sire_bpf = horse.sire.breeding_stats.breeding_potential
    sire_bpf.times do |n|
    ...
    dam_bpf = horse.dam.breeding_stats.breeding_potential
    dam_bpf.times do |n|
    ...
    overall_stat = (parent_stats + grandparent_stats + great_grandparent_stats).fdiv(32 + 16 + 8).round
  ...
  def generate_random_stat(stat, quality)
    min_percent = Config::Breeding.dig(:stat_generation, :quality, quality, :min)
    max_percent = Config::Breeding.dig(:stat_generation, :quality, quality, :max)
  ...
  def determine_foal_quality
    sire_quality = parent_quality_index(sire)
    dam_quality = parent_quality_index(dam)
  ...
  def parent_quality_index(parent)
    quality = parent.quantify

    if parent.breeding_stats&.breeding_potential.to_i < 3
      quality -= 10
    elsif parent.breeding_stats&.breeding_potential.to_i > 8
      quality += 10
    end
  ...
  # gg = 8 * 1, g = 4 * 2, p = 2 * 8 = 32pts
  # gg = 7 * 0.928 + 1 * 1.5, 1 * 3 + 3 * 1.67, 2 * 8
  ...
  def date_of_birth(gender)
    ...
    if breeding.mare.foals.born.count < Config::Breeding.mare_inexperienced_foal_count.to_i
      premie_chance = 13
      late_chance = 26
    elsif breeding.mare.age > Config::Breeding.mare_old_breeding_age
      premie_chance = 14
      late_chance = 22
