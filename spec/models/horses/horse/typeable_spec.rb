describe Horses::Horse::Typeable do
  ...
  describe "#dead?" do
    context "when date of birth == date of death" do
      it "returns true" do
        expect(Horses::Horse::Foal.new(date_of_birth: Date.current, date_of_death: Date.current).dead?).to be true
      end
    end

    context "when date of death is nil" do
      it "returns false" do
        expect(Horses::Horse::Foal.new(date_of_birth: Date.current, date_of_death: nil).dead?).to be false
      end
    end

    context "when date of death > date of birth" do
      it "returns true" do
        expect(Horses::Horse::Foal.new(date_of_birth: Date.current - 1.day, date_of_death: Date.current).dead?).to be true
      end
    end
  end
end
