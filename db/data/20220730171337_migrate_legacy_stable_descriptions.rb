class MigrateLegacyStableDescriptions < ActiveRecord::Migration[7.0]
  def up
    ...
    say_with_time "Migrating legacy descriptions" do
      Legacy::User.where.not(Status: "D").find_each do |legacy_user|
        stable = Account::Stable.find_by(legacy_id: legacy_user.id)
  ...
  def down
    Account::Stable.update_all(description: nil) # rubocop:disable Rails/SkipsModelValidations
  end
end
