module Accounts
  class BudgetTransactionInserter < ApplicationService
    def insert_transaction(stable:, description:, amount:, date:, activity_type: nil, increment_available_balance: false)
      previous_budget = Account::Budget.where(stable:).where(created_at: ...date.end_of_day).recent.first
      attrs = {
        stable:,
        description:,
        amount:,
      ...
      ActiveRecord::Base.transaction do
        ...
        stable.save!
        Account::Budget.where(stable:).where("created_at > ?", date.end_of_day).find_each do |budget|
          budget.update(balance: budget.balance + amount)
        end
