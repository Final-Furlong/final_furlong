module Horse
  class ShipmentsController < AuthenticatedController
      ...
      if result.created?
        ...
        respond_to do |format|
          ...
          format.turbo_stream do
            render turbo_stream: turbo_stream.replace("shipment-form", partial: "horse/shipments/form", locals: { shipment: result.shipment, horse: })
          end
        end
      end
    end
    ...
    def shipment_params
      params.expect(shipment: [:departure_date, :arrival_date, :ending_location, :ending_farm, :mode])
    end
  end
end
