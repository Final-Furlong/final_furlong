module Horses::Racehorse
  class AutoBoardingUpdater
    def call
      horses_updated = 0
      Horses::Racehorse::Boarding.where(start_date: ..(Date.current - Config::Boarding.max_yearly_days.days)).where(end_date: nil).find_each do |boarding|
        Horses::Racehorse::BoardingUpdater.new.stop_boarding(boarding:)
        horses_updated += 1
      end
      Horses::Racehorse::Boarding.current.find_each do |boarding|
      ...
      stables_updated = 0
      Account::Stable.joins(racehorses: :current_boarding).find_each do |stable|
        num_boarded = stable.racehorses.where.associated(:current_boarding).uniq.count

        stable.available_balance = stable.total_balance - (num_boarded * Config::Boarding.daily_fee)
        stable.save
        stables_updated += 1
      end
