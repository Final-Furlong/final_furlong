RSpec.describe Horses::Racehorse::BoardingCreator do
  shared_examples "an unprocessed boarding" do
    it "does not create boarding" do
      expect { described_class.new.start_boarding(horse:) }.not_to change(Horses::Racehorse::Boarding, :count)
    end

    it "returns error" do
      result = described_class.new.start_boarding(horse:)
  ...
  describe "#start_boarding" do
    ...
    context "when horse has been boarded elsewhere for max days" do
      it "allows booking" do
        travel_to Date.new(Date.current.year, 3, 1) do
          ...
          expect { described_class.new.start_boarding(horse:) }.to change(Horses::Racehorse::Boarding, :count).by(1)
        end
      end
    end
    ...
    context "when horse no historical boarding" do
      it "allows booking" do
        expect do
          ...
          expect(result.boarding).to be_a Horses::Racehorse::Boarding
          expect(result.boarding).to have_attributes(horse:, location:, start_date: Date.current, days: 0)
        end.to change(Horses::Racehorse::Boarding, :count).by(1)
      end
    end
    ...
    context "when boarding creation fails" do
      it "allows booking" do
        ...
        allow_any_instance_of(Horses::Racehorse::Boarding).to receive(:save!).and_return false
        result = described_class.new.start_boarding(horse:)
        expect(result.created?).to be false
        expect(result.boarding).to be_a Horses::Racehorse::Boarding
  ...
  def horse
    ...
    @horse = create(:horse)
    create(:racehorse_metadata, horse: @horse, location:, racetrack:)
    @horse
  end
