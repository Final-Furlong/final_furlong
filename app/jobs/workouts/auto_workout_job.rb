  ...
  def perform
    ...
    Racing::TrainingSchedule.with_activities(weekday).where.associated(:training_schedule_horses).distinct.find_each do |schedule|
      ...
      schedule.training_schedule_horses.includes(:horse).where(horse: { status: "racehorse" }).find_each do |training_horse|
        ...
        if activities.activity2.present?
          params[:activity2] = activities.activity2
          params[:distance2] = activities.distance2
        end
        if activities.activity3.present?
          params[:activity3] = activities.activity3
          params[:distance3] = activities.distance3
        end
