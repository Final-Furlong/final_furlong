RSpec.describe Auctions::YearlingConsigner do
  context "when number is 0" do
    it "returns 0 horses" do
      create(:legacy_horse, :yearling, :final_furlong, :sellable)
  ...
  context "when stakes quality is false" do
    it "only returns foals of non-stakes mares" do # rubocop:disable RSpec/ExampleLength
      created = create(:legacy_horse, :yearling, :final_furlong, :sellable, Dam: nil)
      unranked_dam = create(:legacy_horse, :broodmare, :final_furlong)
      unranked = create(:legacy_horse, :yearling, :final_furlong, :sellable, Dam: unranked_dam.ID)
      non_stakes_dam = create(:legacy_horse, :broodmare, :final_furlong)
      create(:legacy_race_record, :winner, Horse: non_stakes_dam.ID)
      non_stakes = create(:legacy_horse, :yearling, :final_furlong, :sellable, Dam: non_stakes_dam.ID)
      stakes_dam = create(:legacy_horse, :broodmare, :final_furlong)
      create(:legacy_race_record, :stakes_placed, Horse: stakes_dam.ID)
      stakes = create(:legacy_horse, :yearling, :final_furlong, :sellable, Dam: stakes_dam.ID)
  ...
  it "only returns horses owned by FF" do
    non_ff = create(:legacy_horse, :yearling, :sellable, Owner: 35)
    ff = create(:legacy_horse, :yearling, :final_furlong, :sellable)
  ...
  it "only returns horses that are sellable" do
    sellable = create(:legacy_horse, :yearling, :final_furlong, :sellable)
    non_sellable = create(:legacy_horse, :yearling, :final_furlong)
