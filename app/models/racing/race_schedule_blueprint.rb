module Racing
  class RaceScheduleBlueprint < Blueprinter::Base
    identifier :id

    fields :number, :distance, :grade, :name, :race_type,
      :age, :female_only, :male_only, :purse

    field :date do |schedule, options|
      I18n.l(schedule.date, format: :default)
    end

    field :entries do |schedule, options|
      schedule.entries.pluck(:horse_id)
    end

    association :track_surface, blueprint: ::Racing::TrackSurfaceBlueprint
  end
end
