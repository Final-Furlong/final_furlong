module CurrentStable
  class BreedersCupNominationsController < AuthenticatedController
    ...
    def create
      ...
      remaining_weanlings = Horses::Horse.weanling.managed_by(Current.stable).joins(sire: :stud_nominations).where(sire: { stud_breeders_cup_nominations: { year: Date.current.year } }).where.missing(:breeders_cup_nomination).exists?
