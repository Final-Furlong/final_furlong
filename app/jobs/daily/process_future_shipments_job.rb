class Daily::ProcessFutureShipmentsJob < ApplicationJob
  ...
  def perform(date: Date.current)
    ...
    unless run_today?
      broodmare_count = 0
      Shipping::BroodmareShipment.scheduled.where(departure_date: today).find_each do |shipment|
    ...
    racehorse_count = 0
    Shipping::RacehorseShipment.scheduled.where(departure_date: today).find_each do |shipment|
      batch.add(Horses::FutureBroodmareShipmentJob.perform_later(id: shipment.id, date:))
      racehorse_count += 1
    end

    Racing::FutureRaceEntry.where(ship_date: today, ship_only_if_horse_is_entered: false).find_each do |entry|
