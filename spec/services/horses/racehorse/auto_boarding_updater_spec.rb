describe Horses::Racehorse::AutoBoardingUpdater do
  describe "#call" do
    it "ends boarding for horses who started 30 days ago" do
      allow(Horses::Racehorse::BoardingUpdater).to receive(:new).and_return mock_updater
    ...
    it "decrements stable balance for all un-ended boarded horses" do
      ...
      stable.update(total_balance: 10_000, available_balance: 10_000)
      described_class.new.call
      expect(stable.reload.available_balance).to eq 9_800
    end
  end
  ...
  def mock_updater
    @mock_updater ||= instance_double(Horses::Racehorse::BoardingUpdater, stop_boarding: true)
  end
