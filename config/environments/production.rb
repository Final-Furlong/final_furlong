...
Rails.application.configure do
  ...
  config.lograge.enabled = ENV.fetch("USE_LOGRAGE", "true") == "true"
  ...
  # config.active_job.queue_name_prefix = "final_furlong_production"
  ...
  config.action_mailer.default_url_options = { host: "www.finalfurlong.org" }
  ...
  config.hosts = [
    ...
    /.*\.finalfurlong\.org/ # Allow requests from subdomains like `www.finalfurlong.org`
  ]

  # Skip DNS rebinding protection for the default health check endpoint.
  config.host_authorization = { exclude: ->(request) { ["/health/app", "/health/upstream"].include?(request.path) } }
end
