RSpec.describe Auctions::BroodmareConsigner do
  context "when number is 0" do
    it "returns 0 horses" do
      create(:legacy_horse, :broodmare, :final_furlong, :sellable)
  ...
  context "when min age = max age" do
    it "only returns horses of that age" do
      younger = create(:legacy_horse, :broodmare, :final_furlong, :sellable, DOB: Date.current - 2.years)
      older = create(:legacy_horse, :broodmare, :final_furlong, :sellable, DOB: Date.current - 3.years)
  ...
  context "when stakes quality is false" do
    it "only returns non-silver/gold ranked horses" do # rubocop:disable RSpec/ExampleLength
      unranked = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
      bronze = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
      silver = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
      gold = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
      platinum = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
  ...
  it "only returns horses owned by FF" do
    non_ff = create(:legacy_horse, :broodmare, :sellable, Owner: 35)
    ff = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
  ...
  it "only returns horses that are sellable" do
    sellable = create(:legacy_horse, :broodmare, :final_furlong, :sellable)
    non_sellable = create(:legacy_horse, :broodmare, :final_furlong)
