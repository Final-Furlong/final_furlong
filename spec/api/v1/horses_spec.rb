RSpec.describe Api::V1::Horses do
  describe "GET /api/v1/horses" do
    context "when horse exists" do
      it "returns horse info" do
        horse.update(sire: create(:horse, :stallion), dam: create(:horse, :broodmare))
        get("/api/v1/horses/#{horse.id}", headers: { "Api-Key" => Rails.application.credentials.api.key! })

        expect(response).to have_http_status :ok
        expect(json_body).to eq ::Horses::RacehorseBlueprint.render_as_json(horse).symbolize_keys
      end
    ...
    context "when horse does not exist" do
      it "returns nil" do
        ...
        expect(json_body).to eq({ error: "not_found", detail: "Couldn't find Horses::Horse with 'id'=1" })
      end
    end
